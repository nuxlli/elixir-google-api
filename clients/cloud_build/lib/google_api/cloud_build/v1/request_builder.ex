...
defmodule GoogleApi.CloudBuild.V1.RequestBuilder do
  ...
  """
  @spec decode({:ok, Tesla.Env.t()} | {:error, Tesla.Env.t()}, struct() | false | nil, keyword()) ::
          {:ok, struct()} | {:error, Tesla.Env.t()}
  def decode(result, struct \\ nil, opts \\ [])

  def decode({:ok, %Tesla.Env{status: status} = env}, _struct, _opts)
  ...
  def decode({:ok, %Tesla.Env{body: body}}, struct, dataWrapped: true) do
    Poison.decode(body, as: %GoogleApi.CloudBuild.V1.RequestBuilder.DataWrapper{}, struct: struct)
    |> case do
...
defimpl Poison.Decoder, for: GoogleApi.CloudBuild.V1.RequestBuilder.DataWrapper do
